...
def generate_adata_cell_by_allele(df_sc_CARLIN, min_clone_size=3):
    """
    ...
    X_clone, ref_clone_id, ref_cell_id = cs.hf.get_X_clone_with_reference_ordering(
        df_sc_CARLIN["RNA_id"], df_sc_CARLIN["clone_id"]
    )
    X_clone = X_clone[:, X_clone.sum(0).A.flatten() >= min_clone_size]
